...
BED_FILE=PCHiC_peak_matrix_cutoff5_${chr}_${eQTL_TYPE}.bed
...
do 
    fimo --motif ${mot} --qv-thresh --thresh 0.05 --oc ${INTERM_DIR}/motif/${mot}_${eQTL_TYPE} ${MOTIF_DIR}/${MOTIF_F} ${INTERM_DIR}/${BED_FILE%.bed}.fa 2>> ${INTERM_DIR}/motif/fimo.output.${eQTL_TYPE}.txt
done


### Re-generate the bed files where TFB motif has been reported

OUTPUT_FILE=${INTERM_DIR}/motif/fimo.output.${CELLTYPE}.${eQTL_TYPE}.${chr}.bed
rm $OUTPUT_FILE

for mot in ${MOTIF_LIST}
do
	cat ${INTERM_DIR}/motif/${mot}_${eQTL_TYPE}/fimo.txt | awk '{print $1,$2}' | sed "/#/d" >> ${OUTPUT_FILE}
done
